...
const { select: d3Select, scaleLinear: d3ScaleLinear, arc: d3Arc, area: d3Area, active: d3Active, interpolate: d3Interpolate, easeLinear: d3EaseLinear } = d3;
export default function renderChart(wrapper, curData, options = {}) {
  if (!wrapper) {
    return;
  }
  ...
  const waveColor = options.waveColor || '#5a98d5';
  ...
  const gaugeCircleX = d3ScaleLinear()
    .range([0, 2 * Math.PI])
    .domain([0, 1]);
  ...
  const elementId = _.uniqueId('elementId_');
  ...
  const waveGroupEnter = gEnter
    .append('defs')
    .append('clipPath')
    .attr('id', 'clipWave' + elementId)
    .attr('transform', 'translate(' + waveGroupXPosition + ',' + waveRiseScale(startValue) + ')');
  waveGroupEnter.append('path').attr('class', 'wave-clip-path').attr('d', clipArea(data)).attr('T', 0);
  svgMerge
    .select('clipPath')
    .transition()
    .duration(waveRiseTime)
  ...
  const ggEnter = gEnter.append('g').attr('clip-path', 'url(#clipWave' + elementId + ')');
  ggEnter.append('circle').attr('cx', radius).attr('cy', radius).attr('r', fillCircleRadius).style('fill', waveColor);
  ...
  function animateWave() {
    ...
    const T = wave.attr('T');
    wave.attr('transform', 'translate(' + waveAnimateScale(T) + ',0)');
    wave
      .transition()
  ...
  animateWave();
  function updater(data = curData, color = waveColor) {
    const fillPercent = Math.max(minValue, Math.min(maxValue, data)) / maxValue;
    svgMerge
      .select('clipPath')
      .transition()
      .duration(waveRiseTime)
      .attr('transform', 'translate(' + waveGroupXPosition + ',' + waveRiseScale(fillPercent) + ')');
    ggEnter.append('circle').attr('cx', radius).attr('cy', radius).attr('r', fillCircleRadius).style('fill', color);
  }
  return updater;
}
